...
@property (nonatomic, assign) CGFloat       iceRadius;
@property (nonatomic, assign) UIRectCorner  roundingCorners;
@property (nonatomic, assign) CGFloat       iceBorderWidth;
@property (nonatomic, assign) BOOL          iceHadAddObserver;
@property (nonatomic, assign) BOOL          iceIsRounding;

@property (nonatomic, strong) UIColor      *iceBorderColor;
...
- (instancetype)initWithRoundingRectImageView {
    if (self = [super init]) {
        [self ice_cornerRadiusRoundingRect];
    }
    return self;
}
...
- (instancetype)initWithCornerRadiusAdcance:(CGFloat)cornerRadius rectCornerType:(UIRectCorner)rectCornerType {
    if (self = [super init]) {
        [self ice_cornerRadiusAdvance:cornerRadius rectCornerType:rectCornerType];
    }
    return self;
}
 ...
 */
- (void)ice_cornerRadiusWithImage:(UIImage *)image cornerRadius:(CGFloat)cornerRadius rectCornerType:(UIRectCorner)rectCornerType {
 ...
 */
- (void)ice_cornerRadiusWithImage:(UIImage *)image cornerRadius:(CGFloat)cornerRadius rectCornerType:(UIRectCorner)rectCornerType backgroundColor:(UIColor *)backgroundColor {
    CGSize size = self.bounds.size;
    CGFloat scale = [UIScreen mainScreen].scale;
    CGSize cornerRadii = CGSizeMake(cornerRadius, cornerRadius);
...
- (void)zy_LayoutSubviews {
    ...
    if (self.zyIsRounding) {
        [self ice_cornerRadiusWithImage:self.image cornerRadius:self.frame.size.width/2 rectCornerType:UIRectCornerAllCorners];
    } else if (0 != self.zyRadius && 0 != self.roundingCorners && nil != self.image) {
        [self ice_cornerRadiusWithImage:self.image cornerRadius:self.zyRadius rectCornerType:self.roundingCorners];
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if ([keyPath isEqualToString:@"image"]) {
        ...
        [self validateFrame];
        if (self.zyIsRounding) {
            [self ice_cornerRadiusWithImage:newImage cornerRadius:self.frame.size.width/2 rectCornerType:UIRectCornerAllCorners];
        } else if (0 != self.zyRadius && 0 != self.roundingCorners && nil != self.image) {
            [self ice_cornerRadiusWithImage:newImage cornerRadius:self.zyRadius rectCornerType:self.roundingCorners];
        }
    }
}
